...
export default function StarterKitClient() {
  ...
  const [individualProject, setIndividualProject] = useState<any>(null); // Projet individuel avec blocs complets
    ...
    if (route === 'blog' && slug) {
      ...
      if (projectSlug) {
        const loadProject = async () => {
          try {
            const response = await fetch(`/api/content/project/${projectSlug}`, {
              cache: 'no-store', // Pas de cache pour avoir les dernières modifications
              headers: {
            ...
            if (response.ok) {
              const data = await response.json();
              // Mettre à jour le projet individuel avec les blocs complets
              setIndividualProject(data.project);
              // Mettre à jour aussi fullContent pour la cohérence
              setFullContent({
                work: {
                  ...metadata.work,
                  adminProjects: metadata.work?.adminProjects?.map((p: any) =>
                    (p.slug === projectSlug || p.id === projectSlug) ? data.project : p
                  ) || [data.project],
      ...
      // Sur les listes (blog ou work sans slug), réinitialiser fullContent et individualProject
      setFullContent(null);
  ...
  if (route === 'home') {
    ...
    // Pour les pages de projet individuelles, utiliser le projet individuel chargé (avec blocs) ou chercher dans le contenu
    const pathSegments = pathname?.split('/').filter(Boolean) || [];
    const slug = pathSegments[1] || '';
    // Priorité au projet individuel chargé depuis l'API (avec blocs complets)
    if (individualProject && (individualProject.slug === slug || individualProject.id === slug)) {
      individualItem = individualProject;
    } else {
      // Fallback : chercher dans fullContent ou content
        ...
        {shouldShowReveal && content && (content.nav?.logo || content.nav?.logoImage) && (
          ...
          >
            {route === 'home' ? (
              ...
              // Page de projet individuel - afficher uniquement les blocs
