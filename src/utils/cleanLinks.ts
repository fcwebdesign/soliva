 ...
 * @param html - Le HTML contenant les liens
...
export function cleanInternalLinks(html: string): string {
  ...
  return html.replace(/<a\s+([^>]*?)href\s*=\s*["']([^"']+)["']([^>]*?)>/gi, (match: string, beforeHref: string, href: string, afterHref: string) => {
 ...
 * @param content - L'objet de contenu
 * @returns - L'objet de contenu nettoyé
 */
export function cleanContentLinks(content: any): any {
