...
function SortableItemWithPages({
  ...
  const {
    ...
    transition,
    isDragging
  } = useSortable({ id: item.id || `service-${index}` });
    ...
    >
      ...
      {isOpen && (
        <div className="px-2 pb-2 pt-1 space-y-2 bg-gray-50 border-t border-gray-200">
          ...
          <div>
            <label className="block text-[10px] text-gray-400 mb-0.5">Page de destination</label>
            <Select
              value={item.link || ''}
              onValueChange={(value) => onUpdate({ link: value })}
            >
...
export default function ServicesSpotlightBlockV2Editor({ data, onChange, compact = false }: { data: ServicesSpotlightData; onChange: (next: ServicesSpotlightData) => void; compact?: boolean }) {
  ...
  const updateField = (field: keyof ServicesSpotlightData, value: any) => {
    onChange({
      ...blockData,
      [field]: value,
    });
  ...
  const addItem = () => {
    const next = [
      ...items,
      { id: `service-${Date.now()}`, title: 'Nouveau service', indicator: '[ Nouveau ]', image: { src: '' } },
    ];
    updateField('items', next);
  };
  ...
  if (compact) {
    return (
      <div className="space-y-2">
        ...
        <div>
          <label className="block text-[10px] text-gray-400 mb-1">Taille des items</label>
          <select
            className="w-full px-2 py-1.5 text-[13px] leading-normal font-normal border border-gray-200 rounded focus:border-blue-400 focus:outline-none transition-colors"
            value={blockData.itemHeadingVariant || 'medium'}
            onChange={(e) => updateField('itemHeadingVariant', e.target.value as 'small' | 'medium' | 'large')}
          >
        ...
        <div className="flex items-center gap-2">
          <Switch
            id="showIndicatorCompact"
            checked={blockData.showIndicator !== false}
            onCheckedChange={(checked) => updateField('showIndicator', checked)}
            className="h-4 w-7 [&>span]:h-3 [&>span]:w-3 [&>span]:data-[state=checked]:translate-x-3"
          />
          <label htmlFor="showIndicatorCompact" className="text-[10px] font-medium">Afficher l'indicateur</label>
        </div>

        <div className="space-y-2">
          <div>
            <label className="text-[10px] text-gray-400">Services ({items.length})</label>
          </div>
              ...
              <div className="space-y-1">
                {items.length === 0 ? (
                  ...
                  items.map((item, index) => {
                    const itemId = item.id || `service-${index}`;
                    const isOpen = openItemId === itemId;

                    return (
                      <SortableItemWithPages
                        key={itemId}
                        item={item}
                        index={index}
                        isOpen={isOpen}
                        onToggle={() => setOpenItemId(isOpen ? null : itemId)}
                        onUpdate={(updates) => updateItem(index, updates)}
                        onRemove={() => removeItem(index)}
                        onToggleVisibility={() => toggleItemVisibility(index)}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Switch
          ...
          className="h-4 w-7 [&>span]:h-3 [&>span]:w-3 [&>span]:data-[state=checked]:translate-x-3"
        />
        <Label htmlFor="showIndicator" className="text-xs font-medium">Afficher l'indicateur</Label>
      </div>

      <div className="flex items-center justify-between pt-2">
        <div className="space-y-0.5">
          <Label className="text-xs text-gray-600">Services</Label>
          <p className="text-[11px] text-gray-400">Titre + indicateur + image spotlight</p>
        </div>
        ...
        {items.map((item, index) => (
          <div key={item.id || index} className="border border-gray-200 rounded-lg p-3 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)] space-y-3">
            <div className="flex items-center justify-between gap-2">
              <div className="text-[12px] font-medium text-gray-700">Service #{index + 1}</div>
              <div className="flex items-center gap-2">
                <Button
                  variant="ghost"
                  size="sm"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-gray-500">Indicateur</Label>
                <Input
                  value={item.indicator || ''}
